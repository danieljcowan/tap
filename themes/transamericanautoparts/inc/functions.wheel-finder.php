...
add_action( 'rest_api_init', function () {
    register_rest_route( 'tap/v1', '/wheel_finder', array(
        'methods' => 'POST',
        'callback' => 'api_wheel_finder',
    ) );
} );

function api_wheel_finder(WP_REST_Request $request) {
...
// THIS IS THE PART NUBMER VERSION OF THE WHEEL FINDER


add_action( 'rest_api_init', function () {
    register_rest_route( 'tap/v1', '/wheel_finder_part_number', array(
        'methods' => 'POST',
        'callback' => 'api_wheel_finder_part_number',
    ) );
} );

function api_wheel_finder_part_number(WP_REST_Request $request) {
    ...
    $args = array(
            ...
            'tax_query' =>  array(
                array(
                    'taxonomy' => 'part_qualifier',
                    'field'    => 'id',
                    'terms'    => array('23154', '23104')
                )
            )
        );
    $wheels = get_posts( $args );
