...
function markerio_enqueue_admin_script($hook)
{
  ...
  if ('toplevel_page_marker-io-plugin' != $hook) {
    return;
  }

  wp_enqueue_style('markerio_style', plugin_dir_url(__FILE__) . 'dist/styles.css', [], '1.0');
  wp_enqueue_script('markerio_script', plugin_dir_url(__FILE__) . 'dist/scripts.js', [], '1.0');
}
...
function markerio_insert_code_snippet()
{
  ...
  if (is_admin()) {
    $enable_admin = false; // To re-enable admin setting, uncomment: $options['enable_admin'] === 'false' ? false : true;
...
// Replace footer
function markerio_edit_footer()
{
  add_filter('admin_footer_text', 'markerio_edit_text', 11);
}
function markerio_edit_text($content)
{
  return '<a href="https://help.marker.io" target="_blank">Give feedback</a>';
}
add_action('admin_init', 'markerio_edit_footer');
